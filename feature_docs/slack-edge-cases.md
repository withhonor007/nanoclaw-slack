...
| 边界情况 | 来源章节 | 优先级 | 风险等级 | 路线图任务 | 说明 |
|---|---|---|---|---|---|
| Bot 消息自循环 | §1.2, §6.3 | P0 | 高 | 任务 7 | 无限循环风险，必须在集成阶段解决 |
| 消息子类型重复处理 | §6.4, §9.5 | P0 | 高 | 任务 7 | URL unfurl 触发 message_changed 导致重复响应 |
| 事件重复投递 | §4.5, §9.1, §9.7 | P0 | 高 | 任务 7 | Socket Mode ack 超时重发 + 平台级重复 |
| Slack 429 限流 | §2.2 | P0 | 高 | 任务 8 | chat.postMessage 1次/秒/频道限制 |
| 5xx 服务端错误 | §3.2 | P0 | 中 | 任务 8 | 需有界重试避免消息丢失 |
| Token 撤销/卸载 | §5.4 | P0 | 高 | 任务 9 | 未处理会导致僵尸进程 |

### P1 — 应实现，可在后续波次

| 边界情况 | 来源章节 | 优先级 | 风险等级 | 路线图任务 | 说明 |
|---|---|---|---|---|---|
| Socket 静默失联 | §9.3 | P1 | 高 | 任务 9 | 进程存活但不处理事件 |
| WebSocket pong 超时 | §9.9 | P1 | 中 | 任务 9 | 重连后事件洪峰需幂等处理 |
| too_many_websockets 崩溃 | §9.2 | P1 | 中 | 任务 9 | 旧进程未退出时连接数耗尽 |
| 队列满/排队超时 | §1.1 | P1 | 中 | 延后 | NanoClaw group-queue 已有并发控制 |
| 有状态正则表达式 | §9.6 | P1 | 低 | 任务 7 | 确保 Bolt 注册的正则无 /g /y 标志 |

### P2 — 延后至未来阶段

| 边界情况 | 来源章节 | 优先级 | 风险等级 | 路线图任务 | 说明 |
|---|---|---|---|---|---|
| 交互模式 | §1.3 | P2 | 低 | 延后 | 本阶段不实现 |
| mrkdwn 格式转换 | §4.2 | P2 | 低 | 延后 | 直接发送，不转换 |
| 文件上传 | §4.4 | P2 | 低 | 延后 | 使用占位符文本 |
| 线程级上下文隔离 | §6.2 | P2 | 中 | 延后 | 频道级对话，非线程级 |
| Slack Connect 外部用户 | §4.6 | P2 | 低 | 延后 | 初始版本不响应 |
| assistant.userMessage 无限循环 | §9.8 | P2 | 低 | 延后 | NanoClaw 不使用 Assistant API |
| HTTP 模式重试头 | §9.10 | P2 | 低 | 延后 | NanoClaw 使用 Socket Mode |