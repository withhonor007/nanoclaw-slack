...
## Key sections
 **readEnvFile call**: Must include `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, `SLACK_SIGNING_SECRET`, and `SLACK_ONLY` in the keys array. NanoClaw does NOT load `.env` into `process.env` — all `.env` values must be explicitly requested via `readEnvFile()`.
 ...
 **SLACK_SIGNING_SECRET**: Read from `process.env` first, then `envConfig` fallback, defaults to empty string
